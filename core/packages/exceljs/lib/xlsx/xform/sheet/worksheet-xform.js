...
const { OleObjectsXform } = require('./ole-objects-xform');
...
class WorkSheetXform extends BaseXform {
  constructor(options) {
    ...
    this.map = {
      ...
      oleObjects: new OleObjectsXform(),
    ...
    switch (name) {
      case 'worksheet': {
        ...
        this.model = {
          ...
          oleObjects: this.map.oleObjects.model || [],
        };
  ...
  reconcile(model, options) {
    ...
    // Process OLE objects
    if (model.oleObjects && model.oleObjects.length) {
      console.log('[Worksheet.reconcile] 处理 oleObjects:', model.oleObjects.length, '个');
      const { parseOLEObject } = require('../../ole-parser');
      model.oleObjects.forEach(oleObj => {
        const rel = rels[oleObj.rId];
        if (rel) {
          oleObj.target = rel.Target;
          oleObj.relType = rel.Type;
          console.log('[Worksheet.reconcile] OLE对象:', oleObj.progId, '-> target:', rel.Target);
        }
        const iconRel = oleObj.iconRId ? rels[oleObj.iconRId] : null;
        if (iconRel) {
          oleObj.iconTarget = iconRel.Target;
          // 关联 EMF 图标 buffer，供上层提取真实文件名
          if (options.media && options.mediaIndex) {
            const iconTargetFile = iconRel.Target.split('/media/')[1];
            if (iconTargetFile) {
              const iconMediaIdx = options.mediaIndex[iconTargetFile];
              if (iconMediaIdx !== undefined && options.media[iconMediaIdx]) {
                oleObj.iconBuffer = options.media[iconMediaIdx].buffer;
                oleObj.iconExtension = options.media[iconMediaIdx].extension;
                console.log('[Worksheet.reconcile] OLE对象图标 buffer 关联成功:', iconTargetFile, 'size:', oleObj.iconBuffer && oleObj.iconBuffer.length);
              }
            }
          }
        }
        // Associate with embeddings data
        if (oleObj.target && options.embeddings) {
          const targetFile = oleObj.target.replace(/^\.\.\//, '');
          const embKey = Object.keys(options.embeddings).find(k => targetFile.endsWith(k));
          if (embKey) {
            const embData = options.embeddings[embKey];
            // Parse with progId now that we know it
            try {
              embData.parsed = parseOLEObject(embData.buffer, oleObj.progId);
              console.log('[Worksheet.reconcile] 解析结果:', {
                fileType: embData.parsed.fileType,
                shortFilename: embData.parsed.shortFilename,
              });
            } catch (e) {
              console.warn('[Worksheet.reconcile] OLE解析失败:', e.message);
            }
            oleObj.embedding = embData;
          } else {
            console.warn('[Worksheet.reconcile] 未找到匹配的embedding:', targetFile);
          }
        }
      });
    }
