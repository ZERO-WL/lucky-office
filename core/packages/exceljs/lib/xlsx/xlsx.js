...
class XLSX {
  ...
  async _processMediaEntry(entry, model, filename) {
    console.log('[ExcelJS] _processMediaEntry: 解析媒体文件:', filename);
    const lastDot = filename.lastIndexOf('.');
    // if we can't determine extension, ignore it
    if (lastDot >= 1) {
      const extension = filename.substr(lastDot + 1);
      const name = filename.substr(0, lastDot);
      console.log('[ExcelJS] _processMediaEntry: 文件名:', name, '扩展名:', extension);
      await new Promise((resolve, reject) => {
        ...
        streamBuf.on('finish', () => {
          ...
          console.log('[ExcelJS] _processMediaEntry: 添加媒体到数组 - index:', model.media.length - 1, medium);
          resolve();
        });
        entry.on('error', error => {
          console.error('[ExcelJS] _processMediaEntry: 错误:', error);
          reject(error);
        });
        entry.pipe(streamBuf);
      });
    } else {
      console.log('[ExcelJS] _processMediaEntry: 忽略文件（无效扩展名）:', filename);
    }
  }

  /**
   * 处理OLE对象文件（xl/embeddings/ 目录下的文件）
   */
  async _processOLEEntry(entry, model, filename) {
    console.log('[ExcelJS] _processOLEEntry: 解析OLE对象:', filename);
    
    const lastDot = filename.lastIndexOf('.');
    const extension = lastDot >= 1 ? filename.substr(lastDot + 1) : 'bin';
    const name = lastDot >= 1 ? filename.substr(0, lastDot) : filename;
    
    await new Promise((resolve, reject) => {
      const streamBuf = new StreamBuf();
      streamBuf.on('finish', async () => {
        const buffer = streamBuf.toBuffer();
        console.log('[ExcelJS] _processOLEEntry: 读取OLE数据', buffer.length, '字节');
        console.log('[ExcelJS] _processOLEEntry: 前16字节hex:',
        ...
        // 仅保存原始buffer，progId未知时不预解析
        // 解析将在reconcile阶段进行（那时已知progId）
        ...
        console.log('[ExcelJS] _processOLEEntry: 添加到embeddings:', filename, '大小:', buffer.length);
        resolve();
      });
      entry.on('error', error => {
        console.error('[ExcelJS] _processOLEEntry: 错误:', error);
        reject(error);
      });
      entry.pipe(streamBuf);
    });
      ...
      if (!entry.dir) {
        ...
        switch (entryName) {
          ...
          default: {
            ...
            // 处理OLE嵌入对象（xl/embeddings/ 目录下）
            match = entryName.match(/xl\/embeddings\/([a-zA-Z0-9]+[.][a-zA-Z0-9]+)$/i);
            if (match) {
              await this._processOLEEntry(stream, model, match[1]);
              break;
            }
