...
function overlayerMousemove(evt) {
  ...
  const {
    rowResizer, colResizer, tableEl, data, table,
  } = this;
  const { rows, cols } = data;
  
  let hoveredButtonIndex = -1;
  let hoveredRowButtonIndex = -1;
  
  if (table.unhideButtons && offsetY <= rows.height) {
    table.unhideButtons.forEach((btn, index) => {
      const arrowSize = 8; // 箭头图标尺寸
      const arrowY = btn.height - 3; // 箭头Y位置（与table.js中一致）
      // 左箭头位置
      const leftArrowX = btn.rightX - 5;
      // 右箭头位置
      const rightArrowX = btn.rightX + 5;
      // 检测是否在左箭头或右箭头区域内
      const leftArrowHit = offsetX >= leftArrowX - arrowSize && 
                           offsetX <= leftArrowX + arrowSize &&
                           offsetY >= arrowY - arrowSize && 
                           offsetY <= arrowY + arrowSize;
      const rightArrowHit = offsetX >= rightArrowX - arrowSize && 
                            offsetX <= rightArrowX + arrowSize &&
                            offsetY >= arrowY - arrowSize && 
                            offsetY <= arrowY + arrowSize;
      if (leftArrowHit || rightArrowHit) {
        hoveredButtonIndex = index;
      }
    });
  }
  
  if (table.unhideRowButtons && offsetX <= cols.indexWidth) {
    table.unhideRowButtons.forEach((btn, index) => {
      const arrowSize = 8; // 箭头图标尺寸
      const rightEdgeX = cols.indexWidth - 5; // 箭头X位置（与table.js中一致）
      // 上箭头位置
      const topArrowY = btn.bottomY - 5;
      // 下箭头位置
      const bottomArrowY = btn.bottomY + 5;
      // 检测是否在上箭头或下箭头区域内
      const topArrowHit = offsetY >= topArrowY - arrowSize && 
                          offsetY <= topArrowY + arrowSize &&
                          offsetX >= rightEdgeX - arrowSize && 
                          offsetX <= rightEdgeX + arrowSize;
      const bottomArrowHit = offsetY >= bottomArrowY - arrowSize && 
                             offsetY <= bottomArrowY + arrowSize &&
                             offsetX >= rightEdgeX - arrowSize && 
                             offsetX <= rightEdgeX + arrowSize;
      if (topArrowHit || bottomArrowHit) {
        hoveredRowButtonIndex = index;
      }
    });
  }
  
  if (hoveredButtonIndex !== -1) {
    table.hoveredButtonIndex = hoveredButtonIndex;
    table.hoveredRowButtonIndex = -1;
    table.render();
    return;
  } else if (hoveredRowButtonIndex !== -1) {
    table.hoveredRowButtonIndex = hoveredRowButtonIndex;
    table.hoveredButtonIndex = -1;
    table.render();
    return;
  } else if (table.hoveredButtonIndex !== -1 || table.hoveredRowButtonIndex !== -1) {
    table.hoveredButtonIndex = -1;
    table.hoveredRowButtonIndex = -1;
    table.render();
  }
...
function overlayerMousedown(evt) {
  ...
  if (table.unhideButtons) {
    const clickedButton = table.unhideButtons.find((btn) => {
      const arrowSize = 8;
      const arrowY = btn.height - 3;
      const leftArrowX = btn.rightX - 5;
      const rightArrowX = btn.rightX + 5;
      const leftArrowHit = offsetX >= leftArrowX - arrowSize && 
                           offsetX <= leftArrowX + arrowSize &&
      ...
      const rightArrowHit = offsetX >= rightArrowX - arrowSize && 
                            ...
                            offsetY >= arrowY - arrowSize && 
                            offsetY <= arrowY + arrowSize;
      return leftArrowHit || rightArrowHit;
    });
    
    if (clickedButton) {
      for (let i = clickedButton.startCol; i <= clickedButton.endCol; i++) {
        data.cols.setHide(i, false);
      }
      sheetReset.call(this);
      return;
    }
  ...
  if (table.unhideRowButtons) {
    const clickedRowButton = table.unhideRowButtons.find((btn) => {
      const arrowSize = 8;
      const rightEdgeX = data.cols.indexWidth - 5;
      const topArrowY = btn.bottomY - 5;
      const bottomArrowY = btn.bottomY + 5;
      const topArrowHit = offsetY >= topArrowY - arrowSize && 
                          offsetY <= topArrowY + arrowSize &&
      ...
      const bottomArrowHit = offsetY >= bottomArrowY - arrowSize && 
                             ...
                             offsetX >= rightEdgeX - arrowSize && 
                             offsetX <= rightEdgeX + arrowSize;
      return topArrowHit || bottomArrowHit;
    });
    
    if (clickedRowButton) {
      for (let i = clickedRowButton.startRow; i <= clickedRowButton.endRow; i++) {
        data.rows.setHide(i, false);
      }
      sheetReset.call(this);
      return;
    }
